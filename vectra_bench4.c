...
static void* worker(void *arg) {
    ...
    for (int t = 0; t < ITERS; t++) {
        for (int i = 0; i < MATRIX_SIZE; i += 2) { // processa 2 células por vez
            VectraCell *c0 = &matrix[core][i];
            VectraCell *c1 = &matrix[core][i+1];
            __builtin_prefetch(c0 + 2, 1, 3);
            __builtin_prefetch(c1 + 2, 1, 3);

            // loop unrolling 8 sub-estados
            for (int s = 0; s < 8; s++) {
